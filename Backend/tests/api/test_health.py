"""
Pytest tests for FastAPI health endpoints.
"""
import pytest
...
def test_tokens_health_endpoint(client):
...
def test_auth_status_endpoint_failure(client):
    ...
    # Patch the function
    with patch("test_health.mock_get_auth_status", return_value=mock_status):
        response = client.get("/health/auth-status")
        assert response.status_code == 200
        assert response.json()["jwt_secret_configured"] is False
        assert response.json()["status"] == "warning"

def test_auth_debug_endpoint_with_valid_user(client):
...
def test_auth_debug_endpoint_without_authentication(client):
